...
unsigned int hash(icl_hash_t* table, void*key){
    if(table)
        return (table->hash_function(key))%table->nbuckets;
    else return 0;
}
 ...
 *
 * @returns -1 generic failure, 0 key already exists, 1 success.
 */
 ...
 *
 * @returns 0 key not found, -1 on failure, 1 key deleted.
 */
